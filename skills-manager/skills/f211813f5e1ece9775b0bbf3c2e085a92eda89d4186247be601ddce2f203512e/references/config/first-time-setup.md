...
This is optional onboarding for users who ask to configure or save translation preferences. Missing EXTEND.md is not a reason to pause a translation: follow the current request, existing preferences if any, and the Defaults in SKILL.md. Ask only about a material ambiguity that cannot be safely inferred.

Do not save persistent preferences without the user's request. Reuse choices already supplied, and ask only for unresolved preferences the user wants to configure.

## Setup Flow

```
User requests preference setup
        |
        v
+---------------------+
| Resolve preferences |
...
Use the available question interface within its supported limits, or ask concisely in text. The following is a question bank, not a mandatory questionnaire; skip values already known and accept documented defaults for nonessential choices.
...
Users can edit EXTEND.md directly or ask to update saved preferences. Deleting it restores request-based/default behavior; it does not trigger mandatory onboarding.
