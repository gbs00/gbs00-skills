...
| Not found | Use the current request and Defaults below; briefly state material assumptions and translate without preference setup. |
...
Missing EXTEND.md does not block translation. Apply the current request/CLI flags, then saved preferences, then Defaults. Ask only when a missing detail materially changes the requested translation and cannot be safely inferred (for example, an unspecified target language when the source is already in the default language).

Only run [optional setup](references/config/first-time-setup.md) when the user asks to configure or save preferences. Reuse answers already in the conversation; do not require a five-question interview or create persistent preferences just to translate once. Use an available question interface or a concise plain-text question, not an unavailable tool as a gate.

## Defaults

All configurable values in one place. The current request/CLI flags take priority over EXTEND.md, which takes priority over these defaults.
