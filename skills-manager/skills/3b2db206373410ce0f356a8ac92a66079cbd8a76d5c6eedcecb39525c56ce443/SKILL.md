...
2. **时间建议与辅助决策（核心体验）**
   - **有明确时间点**（如`明早10点`）：调用相关工具（如 `lark-cli calendar +freebusy` [lark-calendar-freebusy](references/lark-calendar-freebusy.md)）先查询该时间段参会人的忙闲状态（注：若参会人已有日程的 RSVP 状态为拒绝，则认为该时段为空闲）。若均无冲突，用户已给出的明确时间视为本次时间选择，直接创建，不重复询问；若有冲突，提示用户冲突情况并询问是否继续创建或重新选择时间。
...
3. **非阻断式执行**
   - 用户已明确指定时间且无冲突，或已确认推荐时间选项后，执行 `lark-cli calendar +create --summary "..." --start "..." --end "..." --attendee-ids ...`
