...
7. Complete one real conversation through extraction and read-back before continuing the batch. This is part of the authorized run, not another approval step. For an automation, keep this Skill as the workflow source; the scheduled prompt should supply the target and scope instead of copying field/status rules.
...
`discovered -> extracted -> create_pending -> created(record_id) -> verified -> unpinned -> unpin_verified`

Use a stable conversation ID or URL when the page exposes one. Otherwise retain enough exact chat evidence to re-identify the same conversation (recruiter, job and earliest timestamp); company/job/date alone is not a deduplication key. Persist each checkpoint to automation memory, or a task-local ledger for manual runs, before proceeding to the next external action.
...
5. Checkpoint the exact conversation and submitted nine-field payload as `create_pending`, then create a new Lark record with `+record-upsert` without `--record-id`.
6. Immediately checkpoint the returned record ID as `created`, then verify it with `+record-get`. Compare all nine fields and the complete description against the submitted payload, allowing only documented normalization such as single-select arrays.
...
Do not perform generic duplicate searches. On resume, reuse a checkpointed record ID only for the exact pending conversation from the interrupted transaction, whether its last state was `created` or `verified`. Re-run `+record-get` and compare against the saved payload; if complete, checkpoint `verified` and continue from pin verification. If verification fails, stop and report the inconsistency.

If creation timed out or was interrupted before a record ID was captured (`create_pending`), the remote outcome is unknown. Do not blindly create again or unpin. First inspect the original command result/run output for its returned ID; if it cannot be recovered, report that one transaction for reconciliation. Repeated company/job names are not evidence that an arbitrary existing record is the matching result.
...
Report the selected, extracted, read-back-verified, unpin-verified, failed and unattempted counts separately; a written record with pending pin verification is not a fully completed transaction. For each record include date, company, job, salary, experience, education, status, record ID, Lark result and pin result. List discrepancies, uncertainty, closed jobs, stopped actions and the recovery checkpoint.
