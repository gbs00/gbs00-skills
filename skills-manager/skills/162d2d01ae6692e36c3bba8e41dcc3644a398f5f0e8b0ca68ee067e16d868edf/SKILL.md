...
If no `design.md` exists, choose a coherent direction from the user's brief and existing project assets. Missing design.md alone does not require a question:
...
3. **No request to browse options?** Choose a suitable palette from [house-style.md](./house-style.md), state important assumptions, and include the direction in the project plan. Ask only about a material brand constraint that cannot be inferred. Do not add a separate design questionnaire before the plan review.
...
Run once for a new multi-scene project, covering all its compositions. Single-scene pieces and trivial edits are exempt. Ground the plan in `design.md` and `house-style.md`, then obtain user approval before construction. An already approved equivalent expanded prompt/storyboard satisfies this gate; do not ask again for each scene, routine implementation choice, or technical fix within that plan. Material changes to the agreed content, scope, cost, or design direction require renewed approval.
