...
Run once per new multi-scene project, not once per component scene. Single-scene pieces and trivial edits are exempt. Reuse an already approved equivalent project plan/storyboard. Expansion is not about lengthening a short prompt — it's about grounding the user's intent against `design.md` and `house-style.md` and producing a consistent intermediate that every downstream agent reads the same way.
...
**For a new multi-scene plan, do not skip the expansion and review.** Single-scene compositions and trivial edits are exempt. An already approved equivalent plan can be reused across its compositions and routine fixes without re-expansion or repeated approval.
...
Only move to construction after the user approves or says to continue. This is one project-level approval for the presented plan, not a new gate for each scene or routine technical fix. If an equivalent plan/storyboard has already been approved, use that approval without asking again. Material changes to content, scope, cost, or design direction need renewed approval. Explicit render requests and any separate paid-service approvals remain separate requirements.
