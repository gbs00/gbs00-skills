...
description: |
  Capture a website and create a HyperFrames video from it. Use when a URL is provided with explicit video intent, such as "turn this into a video", "make a promo from my site", or a video-based social ad/product tour. A bare URL, a request to read or analyze a page, or software installation instructions are not triggers.
---
...
**Gate:** `STORYBOARD.md` exists with beat-by-beat direction and an asset audit table. Before construction, obtain the single project-level plan approval required by the hyperframes skill. An already approved equivalent storyboard covers its component scenes; do not request approval again for each composition.
...
render to MP4 on explicit request, including an explicit MP4/export request in the initial task. Do not ask for that authorization again if its scope is unchanged. If only a preview is delivered, say so; do not claim a rendered video exists. Do not treat `index.html` as the project
