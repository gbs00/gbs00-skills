...
Only run `hyperframes render` when the user **explicitly asks**, e.g. "render it", "make the final", "export the MP4", or "I'm happy, produce the file". An explicit render/export request in the initial task already counts; do not ask again unless its scope changes. Without that authorization, deliver preview + snapshots and label them as a preview, not an exported MP4. Project plan approval and any separate paid-service approval still apply.
